export default class Cpf {
    ...
    isValidLength = (cpf: string) => cpf.length === 11;
    
    allDigitsTheSame = (cpf: string) => cpf.split("").every(c => c === cpf[0]);
    
    extractCheckDigit = (cpf: string) => cpf.substring(cpf.length-2, cpf.length);
